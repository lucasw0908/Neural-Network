...
class NeuralNetwork:
    ...
    def __init__(
        ...
        dactivation_function: Callable | None = None,
        ...
        self.m: list[np.ndarray] = [np.array(0) for _ in range((len(layers) - 1) * 2)]
        self.v: list[np.ndarray] = [np.array(0) for _ in range((len(layers) - 1) * 2)]
        self.beta1 = 0.9
        self.beta2 = 0.999
        self.gd_times = 1
        self.gd_tag = 0
    ...
    def d(self, f: Callable) -> Callable:
        ...
        def df(x):
            return f(x + delta).imag / delta.imag
    ...
    def predict(self, x_tests: np.ndarray, y_tests: np.ndarray) -> list[np.float64]:
        ...
        for i, (x_test, y_test) in enumerate(zip(x_tests, y_tests, strict=False)):
            ...
            print(
                "\rTest {space}{test}/{tests}, Loss: {loss}, Correct: {correct}".format(
    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            for i in range(0, max_trains, batch_size):
                ...
                for x_train, y_train in zip(x_batch, y_batch, strict=False):
                ...
                print(
                    "\rBatch {space}{batch}/{batchs}, Loss: {loss}, Average Loss: {avg_loss}".format(
            ...
            print(
                "\rEpoch {space}{epoch}/{epochs}, Average Loss: {avg_loss}, Time: {time}".format(
