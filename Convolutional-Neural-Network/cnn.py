...
class ConvolutionalNeuralNetwork(NeuralNetwork):
    ...
    def __init__(
        self,
        layers: list[int],
        conv_layer: tuple[int, int],
        activation_function: Callable,
        dactivation_function: Callable | None = None,
        learning_rate: float = 1e-3,
        data_size: tuple[int, int] = (28, 28),
        ...
        self.kernels = [
            [
                np.random.normal(
                    loc=0,
                    scale=np.sqrt(2.0 / ((c_in := 1) * (c_out := conv_layer[0] * conv_layer[1]))),    # noqa: F841
        ...
        self.X: list[list] = []
        self.pooling_maximum: list[list[list]] = []

        print(f"Conv data size: {self.conv_data_size}")

    def calc_conv_data_size(self) -> int:

        def calc(ds: tuple[int, int]) -> tuple[int, int]:
        ...
        ds = self.data_size
        for _ in range(self.conv_layer[0]):
            ds = calc(ds)
    ...
    def bconv(self, x: np.ndarray, kernel: np.ndarray) -> np.ndarray:
        ...
        padded = np.pad(
            x, ((kernel.shape[0] - 1, kernel.shape[0] - 1), (kernel.shape[1] - 1, kernel.shape[1] - 1)),
            mode="constant"
        )
    ...
    def max_pooling(self, x: np.ndarray, pooling_maximum: list[list[list]]) -> np.ndarray:
